...
import { ServiceRequest, RequestStatus, RTConfig } from '../types';
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 bg-slate-50/30">
           <div className="relative w-full sm:w-64">
            <input 
              type="text" 
              placeholder="Cari warga atau ID..." 
              className="w-full pl-9 pr-4 py-2 bg-white border border-slate-200 rounded-lg text-xs focus:ring-2 focus:ring-blue-500 outline-none"
              value={searchTerm}
              ...
              {filtered.length > 0 ? filtered.map((req) => (
                <tr key={req.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    ...
                    <p className="text-[10px] text-slate-400 mt-0.5 italic">{req.createdAt}</p>
                  </td>
                  <td className="px-6 py-4">
